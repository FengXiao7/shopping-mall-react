...
const ShopCart = () => {
    ...
    const getCartList = async () => {
        ...
        if (result.code === 200 && result.data.length > 0) {
            ...
            inputRef.current = []
            result.data[0].cartInfoList.forEach(c => {
                inputRef.current.push({ skuNum: c.skuNum, skuPrice: c.skuPrice })
            })
        }
    }
    //购物车商品总价
    ...
    //改变产品数量
    //type有三种，对应+,-和直接输入; cartInfo商品信息，disNum相对改变数目,index对应ref数组索引
    const hander = throttle(function (type, cartInfo, disNum,index) {
        //该标志位为false时，不必发送请求
        let flag = true;
                                        ...
                                        </li>
                                        {
                                            ...
                                            <>
                                                ...
                                                <li className={style['cart-list-con5']}>
                                                    <button className={style.mins} onClick={() => hander('minus', cartInfo, -1,index)}>-</button>

                                                    <input autoComplete="off" type="text" minnum="1" className={style.itxt} 
                                                        defaultValue={inputRef.current[index].skuNum}
                                                        ref={inputRef}
                                                    />
                                                    <button className={style.plus} onClick={() => hander('plus', cartInfo, 1,index)}>+</button>
                                                </li>
